...
# ---------------------------------------------------------------------------
# AIRLINE LOGO PROXY
# The frontend can display logos directly via <img> (no CORS needed), but to
# embed them into a downloadable PNG/PDF it must read the raw bytes via fetch()
# — which requires CORS headers the logo host doesn't send. This endpoint
# fetches the logo server-side from airlinelogos.aero and re-serves it WITH
# CORS headers, so the frontend can base64-encode it for html2canvas.
# ---------------------------------------------------------------------------
...
# In-memory cache: { "EK": (bytes, content_type) } — one fetch per code per session
_logo_cache = {}
...
def airline_logo(airline_code):
    """
    Proxy an airline logo with CORS headers so it can be embedded in downloads.
    GET /api/logo/EK  ->  Emirates SVG (Access-Control-Allow-Origin: *)
    Returns 404 if the logo can't be fetched, so the frontend falls back to a
    colored badge.
    """
